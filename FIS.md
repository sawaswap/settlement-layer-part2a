...
*Known limitation (accepted).* The window is derived from time alone, so within the TW3 span it cannot distinguish a **claimed** transaction (bound for `invokeDRP`/`expireTW3`) from an **unclaimed** one (bound for `expireTW2` default-reverse) — both list under TW3. Resolving the two would require reading each row's claim status, an extra on-chain call per row that the no-backend scan deliberately avoids; the "Claim on record" field on the Monitor/Dispute detail view disambiguates a specific STID when needed. The ambiguity is shipped knowingly.
